...
-(void)initSet{
    self.xWidth = SCREENWidth - 50; // k线图宽度
    self.yHeight = 150; // k线图高度
    self.bottomBoxHeight = 44; // 底部成交量图的高度
    self.kLineWidth = 5;// k线实体的宽度
    self.kLinePadding = 1; // k实体的间隔
    self.req_type = @"w"; // 日K线类型
    self.endDate = [NSDate date];
    self.req_freq = @"000001.SS"; // 股票代码 规则是：沪股代码末尾加.ss，深股代码末尾加.sz。如浦发银行的代号是：600000.SS
...
-(void)drawBox{
    // 画个k线图的框框
    if (mainboxView==nil) {
        mainboxView = [[UIView alloc] initWithFrame:CGRectMake((SCREENWidth-self.xWidth)*0.8, 20, self.xWidth, self.yHeight)];
...
-(void)drawLine{
    ...
    WEAKSELF
    getdata = [getdata initWithUrl:[self changeUrl]completeBlock:^(NSArray *d, NSArray *c) {
        weakSelf.data = d ;
        weakSelf.category = c ;
        // 开始画K线图
        [weakSelf drawBoxWithKline];
    }];
