        ...
        [deletedGroup exit:^(id resultObject, NSError *error) {
          ...
          if (error == nil) {
            ...
            [JMSGConversation deleteGroupConversationWithGroupId:deletedGroup.gid];
              UIViewController* rootViewCotroller = self.sendMessageCtl.superViewController;
              if (rootViewCotroller) {
                  [self.navigationController popToViewController:rootViewCotroller animated:YES];
              }
              else{
...
-(void)createChatGroup:(NSArray*)persons{
    if (_conversation.conversationType == kJMSGConversationTypeSingle) {
        [self createGroupWithAlertView:persons];
    } else {
        __weak __typeof(self)weakSelf = self;
        [MBProgressHUD showMessage:@"获取成员信息" toView:self.view];
        [((JMSGGroup *)(self.conversation.target)) addMembersWithUsernameArray:persons appKey:JMESSAGE_APPKEY
                                                             completionHandler:^(id resultObject, NSError *error) {
                                                                 [MBProgressHUD hideAllHUDsForView:weakSelf.view animated:YES];
                                                                 if (error == nil) {
...
- (void)createGroupWithAlertView:(NSArray *)persons {
  {
      NSMutableArray* members = [NSMutableArray arrayWithArray:persons];
      [members addObject:((JMSGUser *)self.conversation.target).username];
    [MBProgressHUD showMessage:@"加好友进群组" toView:self.view];
    __block JMSGGroup *tmpgroup =nil;
    typeof(self) __weak weakSelf = self;
    [JMSGGroup createGroupWithName:@"" desc:@"" memberArray:members completionHandler:^(id resultObject, NSError *error) {
      [MBProgressHUD hideAllHUDsForView:self.view animated:YES];
  ...
  if (indexPath.item == _memberArr.count) {// 添加群成员
    ...
    [_groupMemberGrip reloadData];
      ...
      chatFriendChoose.chooseCompleteBlock = ^(NSArray* persons){
          if (persons.count) {
              [weakSelf createChatGroup:persons];
          }
      };
