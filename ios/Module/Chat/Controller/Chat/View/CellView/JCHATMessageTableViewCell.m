...
//      _headView.layer.borderColor = HBColor(243, 243, 243).CGColor ;
    ...
    if (error == nil) {
      ...
      if ([objectId isEqualToString:user.username]) {
        if (data != nil) {
          ...
          [self.headView setImage:[UIImage imageNamed:@"default-avatar"]];
        }
      } else {
      }
    } else {
...
//    [_messageContent setBubbleSide:isRecive];
    [_messageContent setFrame:CGRectMake(headHeight + 15, 2, contentSize.width, contentSize.height)];
...
//    [_messageContent setBubbleSide:isRecive];
    [_messageContent setFrame:CGRectMake(kApplicationWidth - headHeight - 15 - contentSize.width, 2, contentSize.width, contentSize.height)];
