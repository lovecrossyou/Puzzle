  ...
  [user thumbAvatarData:^(NSData *data, NSString *objectId, NSError *error) {
    if (error == nil) {
      if (data != nil) {
        ...
        _AvatarImgView.image = [UIImage imageNamed:@"default-avatar"];
      }
    } else {
      _AvatarImgView.image = [UIImage imageNamed:@"chat_btn_personal-details"];
    }
  }];
}
...
- (void)setDeleteMember {
  ...
  _AvatarImgView.image = [UIImage imageNamed:@"chat-details_btn_delete-friends-"];
}

- (void)setAddMember {
  _userNameLabel.text = @"";
  _deleteMemberBtn.hidden = YES;
  _AvatarImgView.image = [UIImage imageNamed:@"chat-details_btn_add-friends-"];
}
@end
