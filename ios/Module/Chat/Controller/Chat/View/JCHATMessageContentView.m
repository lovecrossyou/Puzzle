...
static NSInteger const textMessageContentRightOffset = 10;
...
- (void)setMessageContentWith:(JMSGMessage *)message {
  ...
  if (isReceived) { //notice_bg otherChatBg
    maskImage = [UIImage imageNamed:@"otherchat_bg"];
      _textContent.textColor = [UIColor darkGrayColor];
  } else {
    maskImage = [UIImage imageNamed:@"mychat_bg"];
  ...
  switch (message.contentType) {
    ...
    case kJMSGContentTypeUnknown:
      ...
      if (isReceived) {
        [_textContent setFrame:CGRectMake(textMessageContentRightOffset + 15, textMessageContentTopOffset, self.frame.size.width - 2 * textMessageContentRightOffset, self.frame.size.height- 2 * textMessageContentTopOffset)];
      } else {
        [_textContent setFrame:CGRectMake(textMessageContentRightOffset - 15, textMessageContentTopOffset, self.frame.size.width - 2 * textMessageContentRightOffset, self.frame.size.height- 2 * textMessageContentTopOffset)];
      }
